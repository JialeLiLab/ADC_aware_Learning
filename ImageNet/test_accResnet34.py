...
testset = torchvision.datasets.ImageFolder(root='/data/jli990/val', transform=transform)


def test(model, device, batch_size = 256, num_batch = -1):
    model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model', type=str, default='Resnet34_FixQ', help='model name')
    parser.add_argument('-w', '--weight', default=None, help='weights path')
    parser.add_argument('-bs', '--batch-size', type=int, default=512, help='size of each image batch')
