...
class VGG_tiny_MixQ(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Flatten(),
            linear_func(256*4*4, num_classes, bias=True, **qspace)
        )
...
class VGG_tiny_FixQ(nn.Module):
    def __init__(self, num_classes=10, bitw=8, bita='8888888'):
        ...
        print(f'Initialized VGG_tiny_FixQ with bitw: {bitw} and bita: {bita}')
        
        conv_kwargs = {'kernel_size':3, 'stride':1, 'padding':1, 'bias':False}
        
        self.layers = nn.Sequential(
            conv_func(3, 64, ActQ = qm.ImageInputQ, **conv_kwargs, abit=bita[0]), # 0
            nn.BatchNorm2d(64),

            conv_func(64, 64, **conv_kwargs,abit=bita[1]), # 1
            nn.BatchNorm2d(64),
            nn.MaxPool2d(kernel_size=2, stride=2),

            conv_func(64, 128, **conv_kwargs,abit=bita[2]), # 2
            nn.BatchNorm2d(128),

            conv_func(128, 128, **conv_kwargs, abit=bita[3]), # 3
            nn.BatchNorm2d(128),
            nn.MaxPool2d(kernel_size=2, stride=2),

            conv_func(128, 256, **conv_kwargs, abit=bita[4]), # 4
            nn.BatchNorm2d(256),

            conv_func(256, 256, **conv_kwargs, abit=bita[5]), # 5
            nn.BatchNorm2d(256),
            nn.MaxPool2d(kernel_size=2, stride=2),

            nn.Flatten(),
            linear_func(256*4*4, num_classes, bias=True, abit=bita[6])
        )
...
class Resnet_MixQ(nn.Module):
    def __init__(self, num_classes=10, block=BasicBlock):
        ...
        self.layer2 = self._make_layer(block, 128, 2, stride=2, dilate=False)
        self.layer3 = self._make_layer(block, 256, 2, stride=2, dilate=False)
        self.layer4 = self._make_layer(block, 512, 2, stride=2, dilate=False)
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.flatten = nn.Flatten()
        self.fc = linear_func(512, num_classes, bias=True, **qspace)
...
class Resnet_FixQ(nn.Module):
    def __init__(self, num_classes=10, bitw=8, bita='88554444344487585888', block=None):
        ...
        self.layer1 = self._make_layer(block, 64, 2, abit_iter=abit_iter)
        self.layer2 = self._make_layer(block, 128, 2, stride=2, abit_iter=abit_iter)
        self.layer3 = self._make_layer(block, 256, 2, stride=2, abit_iter=abit_iter)
        self.layer4 = self._make_layer(block, 512, 2, stride=2, abit_iter=abit_iter)
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.flatten = nn.Flatten()
        self.fc = qm.QuantActivLinear(512, num_classes, bias=True, abit=next(abit_iter))
